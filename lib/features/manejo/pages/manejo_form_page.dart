...
class _ManejoFormPageState extends State<ManejoFormPage> {
  ...
  void initState() {
    ...
    if (manejo != null) {
      ...
      if (manejo.pesoReferenciaKg != null)
        _pesoReferenciaTexto = manejo.pesoReferenciaKg.toString();
      if (manejo.doseUnidade != null && manejo.doseUnidade!.trim().isNotEmpty)
        _unidadeDose = manejo.doseUnidade!;
  ...
  Future<void> _carregarDados() async {
    try {
      ...
      if (_editando) {
        ...
        if (widget.manejo?.vermifugoId != null)
          _vermifugoSelecionado = _findItem(
            _vermifugos,
            widget.manejo!.vermifugoId,
          );
        if (widget.manejo?.medicamentoId != null)
  ...
  void _calcularDoses() {
    ...
    for (final id in _animaisSelecionados) {
      ...
      if (peso != null && peso > 0)
        calculadas[id] = peso / referencia * doseBase;
    }
    setState(
      () => _dosesCalculadas
        ..clear()
  ...
  void _aplicarRegraDoseTodos() {
    ...
    if (calculadas.length != _animaisSelecionados.length) {
      _mensagem(
        'Informe o peso de cada animal antes de aplicar a dose em lote.',
      );
      return;
    }
    setState(() {
          ...
          DropdownButtonFormField<String>(
            ...
            items: itens
                .map((item) {
                  ...
                  return DropdownMenuItem<String>(
                    value: id,
                    child: Text(
                      principio == null || principio.isEmpty
                          ? nome
                          : '$nome • $principio',
            ...
            onChanged: _salvando
                ...
                : (id) {
                    final item = id == null
                        ? null
                        : itens.firstWhere((x) => x['id']?.toString() == id);
                    onChanged(item);
                  },
          ),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              if (nome.trim().isEmpty) return;
              Navigator.of(dialogContext)
                  .pop({'nome': nome.trim(), 'principio': principio.trim()});
            },
            child: const Text('Cadastrar'),
          ),
        ],
                          ...
                          if (_tipo == TipoManejo.outro) ...[
                            ...
                            TextField(
                              ...
                              decoration: const InputDecoration(
                                ...
                                hintText:
                                    'Ex.: Corte de cascos, limpeza do curral',
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final doseField = TextFormField(
            ...
            decoration: InputDecoration(
              labelText: 'Dose aplicada ($_unidadeDose)',
          ...
          return Column(
            ...
            children: [
              ...
              if (estreito)
                Column(
                  children: [pesoField, const SizedBox(height: 10), doseField],
                )
              else
                Row(
                  children: [
